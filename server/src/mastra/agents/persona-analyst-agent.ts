...
レポートには必ず件数と割合を含め、個人が特定されない形で報告する。
テキストレポートの末尾に、メインエージェントが可視化に使える DISPLAY_DATA ブロックを付与する。
すべてのフィールドは省略可能。質問に直接関連するものだけを含める。
  ...
  "charts": [
    {
      ...
      "xKey": "X軸ラベル",
      "yKey": "Y軸ラベル",
      "data": [{ "X軸ラベル": "値", "Y軸ラベル": 数値 }]
    }
  ],
  "timeline": {
    "title": "タイムラインタイトル",
    "events": [
      { "date": "3/14", "title": "出来事", "description": "詳細", "status": "completed" }
    ]
  },
  "suggestions": ["交通の不満をもっと詳しく", "属性別の傾向は？"]
}
-->
\`\`\`

### 使い分けのガイド
- **charts**: 件数比較や割合の可視化。最大1〜2個
- **timeline**: 出来事の時系列整理。conversation_ended_at ベースで並べる
- **suggestions**: 深掘りの選択肢を提示。村長が次に何を聞けるかをガイド
- テキストで十分伝わるなら DISPLAY_DATA 自体を省略してよい
